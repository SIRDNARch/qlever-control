...
class TestSuite:
    ...
    def __init__(self, name: str, tests, test_count, config: Config, engine_type: str = 'qlever-binaries'):
        """
    ...
    def evaluate_query(
            ...
            result_format: str):
        """
        Evaluates a query result_node based on the expected output and the format.
    ...
    def prepare_test_environment(
        ...
        """
        status = False
        self.engine_manager.remove_index(self.config.command_remove_index)
        index = self.engine_manager.index(self.config.command_index, graph_paths)
        if not index[0]:
            self.update_graph_status(list_of_tests, FAILED, INDEX_BUILD_ERROR)
            self.engine_manager.remove_index(self.config.command_remove_index)
        else:
            server = self.engine_manager.start_server(
    ...
    def run_query_tests(self, graphs_list_of_tests):
        ...
        for graph in graphs_list_of_tests:
            ...
            self.engine_manager.stop_server(self.config.command_stop_server)
    ...
    def compress_json_bz2(self, input_data, output_filename):
        ...
        print("Done writing result_node file: " + output_filename)
