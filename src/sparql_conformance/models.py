...
class TestObject:
    ...
    def __init__(
        ...
        if isinstance(action_node, dict):
            ...
            self.query_file = read_file(os.path.join(self.path, self.group, self.query))
            self.graph_file = read_file(os.path.join(self.path, self.group, self.graph))
        ...
        if isinstance(result_node, dict):
            ...
            self.result_file = read_file(os.path.join(self.path, self.group, self.result))
